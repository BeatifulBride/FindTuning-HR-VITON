...
class CPDataset(data.Dataset):
    ...
    def __getitem__(self, index):
        ...
        # 테스트 주석 처리 해제함. 4
        # 기존에 차원 오류 때문에... 주석 처리했었음
        # 지우면 안된다. 지우면 이상하게 나온다.
        ...
        dim = torch.from_numpy(np.array(image_parse_agnostic)).long().dim()
        if dim == 2:
            parse_agnostic = torch.from_numpy(np.array(image_parse_agnostic)[None]).long()
        elif dim == 4:
            parse_agnostic = torch.from_numpy(np.array(image_parse_agnostic)).long().unsqueeze(0)
        elif dim == 3:
            parse_agnostic = torch.from_numpy(np.array(image_parse_agnostic)).long()
...
class CPDatasetTest(data.Dataset):
    ...
    def __getitem__(self, index):
        # 테스트 self.im_names = init 에서 불러들인 test_train.txt 이다.
        # 테스트 그래서 크기의 수를 560개로 늘렸지만 여전히 오류 발생.
        # print(f"\n im_name 크기?? {len(self.im_names)} 들어온 인덱스 크기? {index}")
        im_name = self.im_names[index]
        # print(f"\n 인덱스로 가져온 im_name 크기? {len(im_name)}")
        c_name = {}
        c = {}
        cm = {}
