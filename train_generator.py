...
from find_tunings.file_utils import file_utils
...
import find_tunings.file_utils
...
name = 'black_train_gen'
model = file_utils.convert_to_wsl_path('D:\\CheckPoints\\condition\\black_condition\\black_condition\\tocg_final.pth')
tocg_root = file_utils.convert_to_wsl_path('D:\\CheckPoints\\condition\\black_condition\\')
# dataroot = '/mnt/d/Test-Set'
dataroot = '/mnt/d/FindTuning/HR-VITON/data'
origin_gen_model = './eval_models/weights/v0.1/gen.pth'
origin_model = './eval_models/weights/v0.1/mtviton.pth'
tensor_root = file_utils.convert_to_wsl_path('D:\\CheckPoints\\condition\\black_condition')
...
def get_opt():
    ...
    # 이미지 크기로 인해 조정함 384 512 보다 더 줄이면 뭔가... 안됨.
    parser.add_argument("--fine_width", type=int, default=384)  # 384 #128
    parser.add_argument("--fine_height", type=int, default=512)  # 512 #170
    parser.add_argument("--radius", type=int, default=20)
    parser.add_argument("--grid_size", type=int, default=5)

    parser.add_argument('--tensorboard_dir', type=str, default=tensor_root, help='save tensorboard infos')
    parser.add_argument('--checkpoint_dir', type=str, default=tensor_root, help='save checkpoint infos')
    parser.add_argument('--tocg_checkpoint', type=str, default=model, help='condition generator checkpoint')
    parser.add_argument('--gen_checkpoint', type=str, default=origin_gen_model, help='gen checkpoint')
    parser.add_argument('--dis_checkpoint', type=str, default=origin_model, help='dis checkpoint')
    ...
    parser.add_argument("--test_dataroot", default=dataroot)
    parser.add_argument("--test_data_list", default="test_pairs.txt")

    # Hyper-parameters
    parser.add_argument('--G_lr', type=float, default=0.00002, help='initial learning rate for adam')
    parser.add_argument('--D_lr', type=float, default=0.00002, help='initial learning rate for adam')
...
def main():
    ...
    # 테스트 500 -> 50 -> 10
    test_dataset = Subset(test_dataset, np.arange(2000))
    ...
    if not opt.GT:
        ...
        tocg = ConditionGenerator(opt=opt, input1_nc=input1_nc, input2_nc=input2_nc, output_nc=13, ngf=96,
                                  norm_layer=nn.BatchNorm2d)
        # Load Checkpoint
        # 테스트, 수정함. 2번째 인수로 tocg를 넘겨주고 있었음. 실제로는 tocg_checkpoint의 경로 필요.
        # 순서변경함...
