...
name = 'black_test_gen'
data_root = file_utils.convert_to_wsl_path('D:\\Test-Set')
# data_root = file_utils.convert_to_wsl_path('D:\\FindTuning\\HR-VITON\\data')
model = './tensorboard/black_condition/black_condition/D_final.pth'
tensor_root = file_utils.convert_to_wsl_path('D:\\CheckPoints')
...
def get_opt():
    ...
    # 테스트 폴더 이름
    parser.add_argument('--test_name', type=str, default=name, help='test name')
    ...
    parser.add_argument("--output_dir", type=str, default=f'/mnt/d/OutPut/{name}')
    parser.add_argument("--datasetting", default="unpaired")

    # 이미지 크기
    # 768-> 384 || 1024 -> 512 ... GPU 메모리 OutOfLange 해결됨....
    parser.add_argument("--fine_width", type=int, default=384)
    parser.add_argument("--fine_height", type=int, default=512)

    # 텐서보드 저장 위치?
    parser.add_argument('--tensorboard_dir', type=str, default=tensor_root,
                        help='save tensorboard infos')
    parser.add_argument('--checkpoint_dir', type=str, default=tensor_root,
                        help='save checkpoint infos')

    # 모델 위치? 학습 받을 모델?
    parser.add_argument('--tocg_checkpoint', type=str, default=model,
